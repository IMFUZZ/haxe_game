...
bool velocityOnly = __o_velocityOnly.Default(false);
{
	...
	HX_STACK_LINE(382)
	::zpp_nape::constraint::ZPP_UserConstraint tmp = ::zpp_nape::constraint::ZPP_UserConstraint_obj::__new(dimensions,velocityOnly);		HX_STACK_VAR(tmp,"tmp");
	HX_STACK_LINE(382)
...
::nape::geom::MatMN UserConstraint_obj::impulse( ){
	...
	HX_STACK_LINE(402)
	{
		...
		while((true)){
			...
			HX_STACK_LINE(403)
			int tmp8 = ret->zpp_inner->n;		HX_STACK_VAR(tmp8,"tmp8");
			HX_STACK_LINE(403)
			...
			HX_STACK_LINE(403)
			Float tmp12 = tmp11->jAcc->__get(i);		HX_STACK_VAR(tmp12,"tmp12");
			HX_STACK_LINE(403)
			ret->zpp_inner->x[tmp10] = tmp12;
		}
	}
	HX_STACK_LINE(405)
