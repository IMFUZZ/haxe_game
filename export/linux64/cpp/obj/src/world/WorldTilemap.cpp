...
HXLINE( 147)								drawX = (helperPointX + (hx::Mod(columnIndex,this->widthInTiles) * scaledWidth));
HXLINE( 148)								drawY = (helperPointY + (::Math_obj::floor(((Float)columnIndex / (Float)this->widthInTiles)) * scaledHeight));
HXLINE( 149)								this->_matrix->identity();
HXLINE( 151)								if ((frame->angle != (int)0)) {
HXLINE( 153)									frame->prepareMatrix(this->_matrix,null(),null(),null());
            								}
HXLINE( 156)								HX_VARI( Float,scaleX) = this->scale->x;
HXLINE( 157)								HX_VARI( Float,scaleY) = this->scale->y;
HXLINE( 159)								if (this->useScaleHack) {
HXLINE( 161)									scaleX = (scaleX + ((Float)(int)1 / (Float)(frame->sourceSize->x * Camera->totalScaleX)));
HXLINE( 162)									scaleY = (scaleY + ((Float)(int)1 / (Float)(frame->sourceSize->y * Camera->totalScaleY)));
            								}
HXLINE( 164)								this->_matrix->scale(scaleX,scaleY);
HXLINE( 165)								this->_matrix->translate(drawX,drawY);
HXLINE( 167)								drawItem->addQuad(frame,this->_matrix,this->colorTransform);
            							}
            						}
HXLINE( 171)						if (::flixel::FlxG_obj::renderBlit) {
HXLINE( 173)							 ::openfl::_legacy::geom::Point _hx_tmp4 = this->_flashPoint;
HXDLIN( 173)							_hx_tmp4->x = (_hx_tmp4->x + this->_tileWidth);
            						}
HXLINE( 175)						columnIndex = (columnIndex + (int)1);
            					}
            				}
HXLINE( 178)				if (::flixel::FlxG_obj::renderBlit) {
HXLINE( 180)					 ::openfl::_legacy::geom::Point _hx_tmp5 = this->_flashPoint;
HXDLIN( 180)					_hx_tmp5->y = (_hx_tmp5->y + this->_tileHeight);
            				}
HXLINE( 182)				rowIndex = (rowIndex + this->widthInTiles);
            			}
            		}
HXLINE( 185)		Buffer->x = (screenXInTiles * this->_scaledTileWidth);
HXLINE( 186)		Buffer->y = (screenYInTiles * this->_scaledTileHeight);
HXLINE( 188)		if (::flixel::FlxG_obj::renderBlit) {
HXLINE( 190)			if (isColored) {
HXLINE( 192)				Buffer->colorTransform(this->colorTransform);
            			}
HXLINE( 194)			Buffer->blend = this->blend;
            		}
HXLINE( 197)		Buffer->dirty = false;
            	}
