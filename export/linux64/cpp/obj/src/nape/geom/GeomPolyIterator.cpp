// Generated by Haxe 3.4.0
#include <hxcpp.h>

#ifndef INCLUDED_nape_geom_GeomPoly
#include <nape/geom/GeomPoly.h>
#endif
#ifndef INCLUDED_nape_geom_GeomPolyIterator
#include <nape/geom/GeomPolyIterator.h>
#endif
#ifndef INCLUDED_nape_geom_GeomPolyList
#include <nape/geom/GeomPolyList.h>
#endif
#ifndef INCLUDED_zpp_nape_util_ZNPList_ZPP_GeomPoly
#include <zpp_nape/util/ZNPList_ZPP_GeomPoly.h>
#endif
#ifndef INCLUDED_zpp_nape_util_ZPP_GeomPolyList
#include <zpp_nape/util/ZPP_GeomPolyList.h>
#endif

HX_DEFINE_STACK_FRAME(_hx_pos_e1b5a56fcbcedc91_178_new,"nape.geom.GeomPolyIterator","new",0x34729534,"nape.geom.GeomPolyIterator.new","nape/geom/GeomPolyIterator.hx",178,0x1868ec5c)
HX_DEFINE_STACK_FRAME(_hx_pos_e1b5a56fcbcedc91_240_hasNext,"nape.geom.GeomPolyIterator","hasNext",0x4e0a6441,"nape.geom.GeomPolyIterator.hasNext","nape/geom/GeomPolyIterator.hx",240,0x1868ec5c)
HX_DEFINE_STACK_FRAME(_hx_pos_e1b5a56fcbcedc91_264_next,"nape.geom.GeomPolyIterator","next",0xafcff99f,"nape.geom.GeomPolyIterator.next","nape/geom/GeomPolyIterator.hx",264,0x1868ec5c)
HX_DEFINE_STACK_FRAME(_hx_pos_e1b5a56fcbcedc91_217_get,"nape.geom.GeomPolyIterator","get",0x346d456a,"nape.geom.GeomPolyIterator.get","nape/geom/GeomPolyIterator.hx",217,0x1868ec5c)
HX_DEFINE_STACK_FRAME(_hx_pos_e1b5a56fcbcedc91_194_boot,"nape.geom.GeomPolyIterator","boot",0xa7e8f8de,"nape.geom.GeomPolyIterator.boot","nape/geom/GeomPolyIterator.hx",194,0x1868ec5c)
namespace nape{
namespace geom{

void GeomPolyIterator_obj::__construct(){
            	HX_STACKFRAME(&_hx_pos_e1b5a56fcbcedc91_178_new)
            	HX_STACK_THIS(this)
HXLINE( 198)		this->zpp_next = null();
HXLINE( 190)		this->zpp_critical = false;
HXLINE( 186)		this->zpp_i = (int)0;
HXLINE( 182)		this->zpp_inner = null();
            	}

Dynamic GeomPolyIterator_obj::__CreateEmpty() { return new GeomPolyIterator_obj; }

void *GeomPolyIterator_obj::_hx_vtable = 0;

Dynamic GeomPolyIterator_obj::__Create(hx::DynamicArray inArgs)
{
	hx::ObjectPtr< GeomPolyIterator_obj > _hx_result = new GeomPolyIterator_obj();
	_hx_result->__construct();
	return _hx_result;
}

bool GeomPolyIterator_obj::_hx_isInstanceOf(int inClassId) {
	return inClassId==(int)0x00000001 || inClassId==(int)0x28f84fde;
}

bool GeomPolyIterator_obj::hasNext(){
            	HX_STACKFRAME(&_hx_pos_e1b5a56fcbcedc91_240_hasNext)
            	HX_STACK_THIS(this)
HXLINE( 241)		this->zpp_inner->zpp_inner->valmod();
HXLINE( 244)		HX_VARI(  ::nape::geom::GeomPolyList,_this) = this->zpp_inner;
HXDLIN( 244)		_this->zpp_inner->valmod();
HXDLIN( 244)		if (_this->zpp_inner->zip_length) {
HXLINE( 244)			_this->zpp_inner->zip_length = false;
HXDLIN( 244)			_this->zpp_inner->user_length = _this->zpp_inner->inner->length;
            		}
HXLINE( 245)		this->zpp_critical = true;
HXLINE( 246)		if ((this->zpp_i < _this->zpp_inner->user_length)) {
HXLINE( 247)			return true;
            		}
            		else {
HXLINE( 250)			{
HXLINE( 251)				this->zpp_next = ::nape::geom::GeomPolyIterator_obj::zpp_pool;
HXLINE( 252)				::nape::geom::GeomPolyIterator_obj::zpp_pool = hx::ObjectPtr<OBJ_>(this);
HXLINE( 253)				this->zpp_inner = null();
            			}
HXLINE( 255)			return false;
            		}
HXLINE( 246)		return false;
            	}


HX_DEFINE_DYNAMIC_FUNC0(GeomPolyIterator_obj,hasNext,return )

 ::nape::geom::GeomPoly GeomPolyIterator_obj::next(){
            	HX_STACKFRAME(&_hx_pos_e1b5a56fcbcedc91_264_next)
            	HX_STACK_THIS(this)
HXLINE( 265)		this->zpp_critical = false;
HXLINE( 266)		return this->zpp_inner->at(this->zpp_i++);
            	}


HX_DEFINE_DYNAMIC_FUNC0(GeomPolyIterator_obj,next,return )

 ::nape::geom::GeomPolyIterator GeomPolyIterator_obj::zpp_pool;

 ::nape::geom::GeomPolyIterator GeomPolyIterator_obj::get( ::nape::geom::GeomPolyList list){
            	HX_GC_STACKFRAME(&_hx_pos_e1b5a56fcbcedc91_217_get)
            	HX_STACK_ARG(list,"list")
HXLINE( 218)		HX_VAR(  ::nape::geom::GeomPolyIterator,ret);
HXDLIN( 218)		if (hx::IsNull( ::nape::geom::GeomPolyIterator_obj::zpp_pool )) {
HXLINE( 219)			::zpp_nape::util::ZPP_GeomPolyList_obj::internal = true;
HXLINE( 220)			HX_VARI_NAME(  ::nape::geom::GeomPolyIterator,ret1,"ret") =  ::nape::geom::GeomPolyIterator_obj::__alloc( HX_CTX );
HXLINE( 221)			::zpp_nape::util::ZPP_GeomPolyList_obj::internal = false;
HXLINE( 218)			ret = ret1;
            		}
            		else {
HXLINE( 225)			HX_VARI(  ::nape::geom::GeomPolyIterator,r) = ::nape::geom::GeomPolyIterator_obj::zpp_pool;
HXLINE( 226)			::nape::geom::GeomPolyIterator_obj::zpp_pool = r->zpp_next;
HXLINE( 218)			ret = r;
            		}
HXLINE( 229)		ret->zpp_i = (int)0;
HXLINE( 230)		ret->zpp_inner = list;
HXLINE( 231)		ret->zpp_critical = false;
HXLINE( 232)		return ret;
            	}


STATIC_HX_DEFINE_DYNAMIC_FUNC1(GeomPolyIterator_obj,get,return )


hx::ObjectPtr< GeomPolyIterator_obj > GeomPolyIterator_obj::__new() {
	hx::ObjectPtr< GeomPolyIterator_obj > __this = new GeomPolyIterator_obj();
	__this->__construct();
	return __this;
}

hx::ObjectPtr< GeomPolyIterator_obj > GeomPolyIterator_obj::__alloc(hx::Ctx *_hx_ctx) {
	GeomPolyIterator_obj *__this = (GeomPolyIterator_obj*)(hx::Ctx::alloc(_hx_ctx, sizeof(GeomPolyIterator_obj), true, "nape.geom.GeomPolyIterator"));
	*(void **)__this = GeomPolyIterator_obj::_hx_vtable;
	__this->__construct();
	return __this;
}

GeomPolyIterator_obj::GeomPolyIterator_obj()
{
}

void GeomPolyIterator_obj::__Mark(HX_MARK_PARAMS)
{
	HX_MARK_BEGIN_CLASS(GeomPolyIterator);
	HX_MARK_MEMBER_NAME(zpp_inner,"zpp_inner");
	HX_MARK_MEMBER_NAME(zpp_i,"zpp_i");
	HX_MARK_MEMBER_NAME(zpp_critical,"zpp_critical");
	HX_MARK_MEMBER_NAME(zpp_next,"zpp_next");
	HX_MARK_END_CLASS();
}

void GeomPolyIterator_obj::__Visit(HX_VISIT_PARAMS)
{
	HX_VISIT_MEMBER_NAME(zpp_inner,"zpp_inner");
	HX_VISIT_MEMBER_NAME(zpp_i,"zpp_i");
	HX_VISIT_MEMBER_NAME(zpp_critical,"zpp_critical");
	HX_VISIT_MEMBER_NAME(zpp_next,"zpp_next");
}

hx::Val GeomPolyIterator_obj::__Field(const ::String &inName,hx::PropertyAccess inCallProp)
{
	switch(inName.length) {
	case 4:
		if (HX_FIELD_EQ(inName,"next") ) { return hx::Val( next_dyn()); }
		break;
	case 5:
		if (HX_FIELD_EQ(inName,"zpp_i") ) { return hx::Val( zpp_i); }
		break;
	case 7:
		if (HX_FIELD_EQ(inName,"hasNext") ) { return hx::Val( hasNext_dyn()); }
		break;
	case 8:
		if (HX_FIELD_EQ(inName,"zpp_next") ) { return hx::Val( zpp_next); }
		break;
	case 9:
		if (HX_FIELD_EQ(inName,"zpp_inner") ) { return hx::Val( zpp_inner); }
		break;
	case 12:
		if (HX_FIELD_EQ(inName,"zpp_critical") ) { return hx::Val( zpp_critical); }
	}
	return super::__Field(inName,inCallProp);
}

bool GeomPolyIterator_obj::__GetStatic(const ::String &inName, Dynamic &outValue, hx::PropertyAccess inCallProp)
{
	switch(inName.length) {
	case 3:
		if (HX_FIELD_EQ(inName,"get") ) { outValue = get_dyn(); return true; }
		break;
	case 8:
		if (HX_FIELD_EQ(inName,"zpp_pool") ) { outValue = zpp_pool; return true; }
	}
	return false;
}

hx::Val GeomPolyIterator_obj::__SetField(const ::String &inName,const hx::Val &inValue,hx::PropertyAccess inCallProp)
{
	switch(inName.length) {
	case 5:
		if (HX_FIELD_EQ(inName,"zpp_i") ) { zpp_i=inValue.Cast< int >(); return inValue; }
		break;
	case 8:
		if (HX_FIELD_EQ(inName,"zpp_next") ) { zpp_next=inValue.Cast<  ::nape::geom::GeomPolyIterator >(); return inValue; }
		break;
	case 9:
		if (HX_FIELD_EQ(inName,"zpp_inner") ) { zpp_inner=inValue.Cast<  ::nape::geom::GeomPolyList >(); return inValue; }
		break;
	case 12:
		if (HX_FIELD_EQ(inName,"zpp_critical") ) { zpp_critical=inValue.Cast< bool >(); return inValue; }
	}
	return super::__SetField(inName,inValue,inCallProp);
}

bool GeomPolyIterator_obj::__SetStatic(const ::String &inName,Dynamic &ioValue,hx::PropertyAccess inCallProp)
{
	switch(inName.length) {
	case 8:
		if (HX_FIELD_EQ(inName,"zpp_pool") ) { zpp_pool=ioValue.Cast<  ::nape::geom::GeomPolyIterator >(); return true; }
	}
	return false;
}

void GeomPolyIterator_obj::__GetFields(Array< ::String> &outFields)
{
	outFields->push(HX_HCSTRING("zpp_inner","\x51","\x65","\x85","\x78"));
	outFields->push(HX_HCSTRING("zpp_i","\xa4","\xfa","\x3f","\x89"));
	outFields->push(HX_HCSTRING("zpp_critical","\xc4","\x9f","\x6b","\xcd"));
	outFields->push(HX_HCSTRING("zpp_next","\x98","\x61","\x7a","\x37"));
	super::__GetFields(outFields);
};

#if HXCPP_SCRIPTABLE
static hx::StorageInfo GeomPolyIterator_obj_sMemberStorageInfo[] = {
	{hx::fsObject /*::nape::geom::GeomPolyList*/ ,(int)offsetof(GeomPolyIterator_obj,zpp_inner),HX_HCSTRING("zpp_inner","\x51","\x65","\x85","\x78")},
	{hx::fsInt,(int)offsetof(GeomPolyIterator_obj,zpp_i),HX_HCSTRING("zpp_i","\xa4","\xfa","\x3f","\x89")},
	{hx::fsBool,(int)offsetof(GeomPolyIterator_obj,zpp_critical),HX_HCSTRING("zpp_critical","\xc4","\x9f","\x6b","\xcd")},
	{hx::fsObject /*::nape::geom::GeomPolyIterator*/ ,(int)offsetof(GeomPolyIterator_obj,zpp_next),HX_HCSTRING("zpp_next","\x98","\x61","\x7a","\x37")},
	{ hx::fsUnknown, 0, null()}
};
static hx::StaticInfo GeomPolyIterator_obj_sStaticStorageInfo[] = {
	{hx::fsObject /*::nape::geom::GeomPolyIterator*/ ,(void *) &GeomPolyIterator_obj::zpp_pool,HX_HCSTRING("zpp_pool","\x81","\x5d","\xd4","\x38")},
	{ hx::fsUnknown, 0, null()}
};
#endif

static ::String GeomPolyIterator_obj_sMemberFields[] = {
	HX_HCSTRING("zpp_inner","\x51","\x65","\x85","\x78"),
	HX_HCSTRING("zpp_i","\xa4","\xfa","\x3f","\x89"),
	HX_HCSTRING("zpp_critical","\xc4","\x9f","\x6b","\xcd"),
	HX_HCSTRING("zpp_next","\x98","\x61","\x7a","\x37"),
	HX_HCSTRING("hasNext","\x6d","\xa5","\x46","\x18"),
	HX_HCSTRING("next","\xf3","\x84","\x02","\x49"),
	::String(null()) };

static void GeomPolyIterator_obj_sMarkStatics(HX_MARK_PARAMS) {
	HX_MARK_MEMBER_NAME(GeomPolyIterator_obj::__mClass,"__mClass");
	HX_MARK_MEMBER_NAME(GeomPolyIterator_obj::zpp_pool,"zpp_pool");
};

#ifdef HXCPP_VISIT_ALLOCS
static void GeomPolyIterator_obj_sVisitStatics(HX_VISIT_PARAMS) {
	HX_VISIT_MEMBER_NAME(GeomPolyIterator_obj::__mClass,"__mClass");
	HX_VISIT_MEMBER_NAME(GeomPolyIterator_obj::zpp_pool,"zpp_pool");
};

#endif

hx::Class GeomPolyIterator_obj::__mClass;

static ::String GeomPolyIterator_obj_sStaticFields[] = {
	HX_HCSTRING("zpp_pool","\x81","\x5d","\xd4","\x38"),
	HX_HCSTRING("get","\x96","\x80","\x4e","\x00"),
	::String(null())
};

void GeomPolyIterator_obj::__register()
{
	hx::Object *dummy = new GeomPolyIterator_obj;
	GeomPolyIterator_obj::_hx_vtable = *(void **)dummy;
	hx::Static(__mClass) = new hx::Class_obj();
	__mClass->mName = HX_HCSTRING("nape.geom.GeomPolyIterator","\x42","\x2b","\xd3","\x74");
	__mClass->mSuper = &super::__SGetClass();
	__mClass->mConstructEmpty = &__CreateEmpty;
	__mClass->mConstructArgs = &__Create;
	__mClass->mGetStaticField = &GeomPolyIterator_obj::__GetStatic;
	__mClass->mSetStaticField = &GeomPolyIterator_obj::__SetStatic;
	__mClass->mMarkFunc = GeomPolyIterator_obj_sMarkStatics;
	__mClass->mStatics = hx::Class_obj::dupFunctions(GeomPolyIterator_obj_sStaticFields);
	__mClass->mMembers = hx::Class_obj::dupFunctions(GeomPolyIterator_obj_sMemberFields);
	__mClass->mCanCast = hx::TCanCast< GeomPolyIterator_obj >;
#ifdef HXCPP_VISIT_ALLOCS
	__mClass->mVisitFunc = GeomPolyIterator_obj_sVisitStatics;
#endif
#ifdef HXCPP_SCRIPTABLE
	__mClass->mMemberStorageInfo = GeomPolyIterator_obj_sMemberStorageInfo;
#endif
#ifdef HXCPP_SCRIPTABLE
	__mClass->mStaticStorageInfo = GeomPolyIterator_obj_sStaticStorageInfo;
#endif
	hx::_hx_RegisterClass(__mClass->mName, __mClass);
}

void GeomPolyIterator_obj::__boot()
{
{
            	HX_STACKFRAME(&_hx_pos_e1b5a56fcbcedc91_194_boot)
HXLINE( 194)		zpp_pool = null();
            	}
}

} // end namespace nape
} // end namespace geom
