...
::String BodyCallback_obj::toString( ){
	...
	HX_STACK_LINE(200)
	::String tmp4 = Array_obj< ::String >::__new().Add(HX_HCSTRING("WAKE","\x24","\x24","\xb3","\x39")).Add(HX_HCSTRING("SLEEP","\xf7","\xf4","\xb2","\xfc"))->__get(tmp3);		HX_STACK_VAR(tmp4,"tmp4");
	HX_STACK_LINE(200)
	hx::AddEq(ret,tmp4);
	HX_STACK_LINE(201)
	::zpp_nape::callbacks::ZPP_Callback tmp5 = this->zpp_inner;		HX_STACK_VAR(tmp5,"tmp5");
	HX_STACK_LINE(201)
	::nape::phys::Body tmp6 = tmp5->body->outer;		HX_STACK_VAR(tmp6,"tmp6");
	HX_STACK_LINE(201)
