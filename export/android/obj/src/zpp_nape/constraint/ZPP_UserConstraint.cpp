		...
		while((true)){
			...
			if ((tmp5)){
				...
				HX_STACK_LINE(307)
				{
					...
					if ((tmp10)){
						HX_STACK_LINE(308)
						tmp6 = b->body->component;
					}
					else{
						HX_STACK_LINE(310)
						::zpp_nape::space::ZPP_Component tmp11 = b->body->component;		HX_STACK_VAR(tmp11,"tmp11");
						HX_STACK_LINE(310)
...
bool ZPP_UserConstraint_obj::preStep( Float dt){
	...
	tmp->__validate();
	HX_STACK_LINE(445)
	Float tmp1 = this->pre_dt;		HX_STACK_VAR(tmp1,"tmp1");
	HX_STACK_LINE(445)
	...
	HX_STACK_LINE(446)
	Float tmp4 = this->pre_dt;		HX_STACK_VAR(tmp4,"tmp4");
	HX_STACK_LINE(446)
	Float tmp5 = (Float(tmp3) / Float(tmp4));		HX_STACK_VAR(tmp5,"tmp5");
	HX_STACK_LINE(446)
	Float dtratio = tmp5;		HX_STACK_VAR(dtratio,"dtratio");
	HX_STACK_LINE(447)
	this->pre_dt = dt;
	HX_STACK_LINE(448)
	this->stepped = true;
	...
	if ((tmp10)){
		HX_STACK_LINE(452)
		bool tmp12 = this->velonly;		HX_STACK_VAR(tmp12,"tmp12");
		HX_STACK_LINE(452)
		bool tmp13 = tmp12;		HX_STACK_VAR(tmp13,"tmp13");
		HX_STACK_LINE(452)
		bool tmp14 = tmp13;		HX_STACK_VAR(tmp14,"tmp14");
	...
	if ((tmp11)){
		...
		if ((tmp14)){
			HX_STACK_LINE(463)
			Float tmp16 = this->lsq(this->bias);		HX_STACK_VAR(tmp16,"tmp16");
			HX_STACK_LINE(463)
			Float tmp17 = tmp16;		HX_STACK_VAR(tmp17,"tmp17");
			HX_STACK_LINE(463)
			Float tmp18 = this->maxError;		HX_STACK_VAR(tmp18,"tmp18");
			HX_STACK_LINE(463)
		...
		HX_STACK_LINE(464)
		{
			...
			while((true)){
				...
				HX_STACK_LINE(464)
				Float tmp20 = biasCoef;		HX_STACK_VAR(tmp20,"tmp20");
				HX_STACK_LINE(464)
				Float tmp21 = -(tmp20);		HX_STACK_VAR(tmp21,"tmp21");
				HX_STACK_LINE(464)
				hx::MultEq(this->bias[i],tmp21);
			}
		}
		HX_STACK_LINE(465)
		Float tmp16 = this->maxError;		HX_STACK_VAR(tmp16,"tmp16");
		HX_STACK_LINE(465)
		this->_clamp(this->bias,tmp16);
	}
	else{
		HX_STACK_LINE(468)
		{
	...
	HX_STACK_LINE(472)
	{
		...
		while((true)){
			...
			int i = tmp15;		HX_STACK_VAR(i,"i");
			HX_STACK_LINE(472)
			Float tmp16 = dtratio;		HX_STACK_VAR(tmp16,"tmp16");
			HX_STACK_LINE(472)
			hx::MultEq(this->jAcc[i],tmp16);
		}
	}
	HX_STACK_LINE(473)
